...
def build_pipeline(predictors: list[str], continuous_base_names: set[str]) -> Pipeline:
    ...
    return Pipeline([("preprocessor", ColumnTransformer(transformers=transformers, remainder="drop")), ("model", estimator)])
...
def main() -> None:
    ...
    predictors = spec["matrix_sets"]["A_only"] + spec["matrix_sets"]["B_only"]
    continuous_base_names = set(spec["continuous_base_names"])
    model_name = "xgboost_A_plus_B_id_isolated"
    df = read_csv("output/tables/21b_wave1_model_input_A_plus_B_id_isolated.csv")
    if df[predictors + [LABEL_COL]].isna().any().any():
        raise RuntimeError("xgboost_A_plus_B_id_isolated 输入数据存在缺失。")
    ...
    final_model.fit(x, y)
    joblib.dump(final_model, PROJECT_ROOT / "output/models/21d_xgboost_A_plus_B_id_isolated.joblib")

    write_dataframe_csv(cv_df, "output/tables/21d_cv_metrics_xgboost_A_plus_B_id_isolated.csv")
    write_dataframe_csv(pd.concat(pred_frames, ignore_index=True), "output/tables/21d_wave1_predictions_xgboost_A_plus_B_id_isolated.csv")
    write_text(
        "output/logs/21d_model_xgboost_id_isolated_summary.txt",
